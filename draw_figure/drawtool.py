...
this script aim to evalute model and draw p-r curve and roc curve .
Author: chenglong Yi
date: 2019-02-15
...
def read_result_file(filepath):
    ...
    for item in lines:
        ...
        if cols == 2:
            ...
            y_score.append(t_list)
    
    return y_label, y_score

def read_result_file_multi(filepath, multicls=False):
...
def draw_pr_curve(filepath):
    '''
    @param filepath : str

    '''

    y_label, y_score = read_result_file(filepath)
...
def draw_pr_curve_multi(filepath, multicls=False):
    '''
    @param filepath: str
    @param multicls: bool
    
    '''
    y_label, y_score = read_result_file_multi(filepath, True)
...
def draw_roc_curve_multi(filepath, multicls=False):
    '''
    @param filepath : str
    @param multicls : bool
...
def test_1():
    filepath = "./res.mnist.pr.txt"
    draw_pr_curve(filepath)
    draw_roc_curve(filepath)

def test_2():
    filepath = "./result.example.1.txt"
    draw_pr_curve_multi(filepath, True)
    draw_roc_curve_multi(filepath, True)



def test_3():
    filepath = "../test_data_01/pred_res_callphone_binary.txt"
    draw_pr_curve(filepath)
    draw_roc_curve(filepath)

def test_4():
    filepath = "../test_data_01/pred_res_smoke_binary.txt"
    draw_pr_curve(filepath)
    draw_roc_curve(filepath)

def test_5():
    filepath = "../test_data_02/pred_res_callphone_binary.txt"
    draw_pr_curve(filepath)
    draw_roc_curve(filepath)

def test_6():
    filepath = "../test_data_02/pred_res_smoke_binary.txt"
    draw_pr_curve(filepath)
    draw_roc_curve(filepath)
